...
def startBubbleSort(data, drawData, stepTime, on_complete, report):
    ...
    def step():
        if i[0] < n - 1:
            if j[0] < n - i[0] - 1:
                colorData[j[0]] = colorData[j[0]+1] = 'white'
                drawData(data, colorData)
                report['comparisons'] += 1
                def after_compare():
                    colorData[j[0]] = colorData[j[0]+1] = 'grey'
                    if data[j[0]] > data[j[0]+1]:
                        data[j[0]], data[j[0]+1] = data[j[0]+1], data[j[0]]
                        colorData[j[0]] = colorData[j[0]+1] = 'red'
                        drawData(data, colorData)
                        report['swaps'] += 1
                        def after_swap():
                            colorData[j[0]] = colorData[j[0]+1] = 'grey'
